# @hishprorg/corrupti-totam

![CI](https://github.com/hishprorg/corrupti-totam/workflows/CI/badge.svg)
[![NPM version](https://img.shields.io/npm/v/@hishprorg/corrupti-totam.svg?style=flat)](https://www.npmjs.com/package/@hishprorg/corrupti-totam)
...
`@hishprorg/corrupti-totam` supports Fastify@3.
`@hishprorg/corrupti-totam` [v1.x](https://github.com/hishprorg/corrupti-totam/tree/1.x)
supports both Fastify@2.

## Install
```
npm i @hishprorg/corrupti-totam
```
...
fastify.register(require('@hishprorg/corrupti-totam'), {
  secret: 'supersecret'
})
...
const fastify = require('fastify')()
const jwt = require('@hishprorg/corrupti-totam')
// secret as a string
...
Optionally you can define global default options that will be used by `@hishprorg/corrupti-totam` API if you do not override them.
...
```
### fastify.jwt.cookie
For your convenience, `request.jwtVerify()` will look for the token in the cookies property of the decorated request. You must specify `cookieName`. Refer to the [cookie example](https://github.com/hishprorg/corrupti-totam#example-using-cookie) to see sample usage and important caveats.
...
The following algorithms are currently supported by [fast-jwt](https://github.com/nearform/fast-jwt) that is internally used by `@hishprorg/corrupti-totam`.
...
```js
const Fastify = require('fastify')
const fjwt = require('@hishprorg/corrupti-totam')
...
```ts
import fastifyJwt, { FastifyJWTOptions } from '@hishprorg/corrupti-totam'
```
...
```ts
// fastify-jwt.d.ts
import "@hishprorg/corrupti-totam"

declare module "@hishprorg/corrupti-totam" {
